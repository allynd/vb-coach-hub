...
function summarizeEvents(events, playerId=null){
  ...
  return {
    ...
    BS:count('block_solo'),BA:count('block_assist'),BE:count('block_error'),
...
function matchSummaryText(g){
  ...
  const lines=players.map(p=>{const s=summarizeEvents(ev,p.id);return `#${p.jersey||'—'} ${p.firstName} ${p.lastName}: K ${s.K}, E ${s.E}, ATT ${s.ATT}, HIT ${fmtPct(s.HIT)}, ACE ${s.ACE}, BS ${s.BS}, BA ${s.BA}, BE ${s.BE}, AST ${s.A}, DIG ${s.D}, DE ${s.DE}, BHE ${s.BHE}, PASS ${s.PASS.toFixed(2)}`;});
