...
const simpleStatTypes = new Set(['serve_ace','serve_in','serve_error','attack_kill','attack_attempt','attack_error','block_solo','set_assist','dig','defense_error','team_point','opp_point']);
...
function summarizeEvents(events, playerId=null){
  ...
  return {
    ...
    D:count('dig'),DE:count('defense_error'),A:count('set_assist'),BHE:count('set_error'),
...
function matchSummaryText(g){
  ...
  const lines=players.map(p=>{const s=summarizeEvents(ev,p.id);return `#${p.jersey||'—'} ${p.firstName} ${p.lastName}: K ${s.K}, E ${s.E}, ATT ${s.ATT}, HIT ${fmtPct(s.HIT)}, ACE ${s.ACE}, AST ${s.A}, DIG ${s.D}, DE ${s.DE}, BHE ${s.BHE}, PASS ${s.PASS.toFixed(2)}`;});
